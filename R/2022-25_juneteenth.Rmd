...
firsts_tbl <- firsts %>%
  ...
  mutate(label = paste(accomplishment, person)) %>% 
  group_by(year) %>% 
  sample_n(1)
```
...
subtitle <- str_to_upper(
  "First African-American ..."
    )
title <- "Major African-American accomplishments in the nineteenth century"
caption <- "Source:"
...
plt_1 <- 
  ...
  geom_label(aes(x = year, y = 0, label = year, fill = category),
             color = "white",
